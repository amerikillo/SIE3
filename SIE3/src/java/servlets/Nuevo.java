...
public class Nuevo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            if (request.getParameter("accion").equals("Guardar")) {
                try {
                    ...
                    while (rsetDatos.next()) {
                        ...
                        con.insertar("insert into tb_compra values (0,'" + F_IndCom + "','" + F_Provee + "','A',curdate(), '" + F_ClaPro + "', '" + F_Piezas + "', '" + F_Costo + "', '" + F_Caja + "', '0', '" + F_Tarimas + "', '" + F_ImpTo + "' ,'" + F_ComTot + "', '" + FolioLote + "', '" + F_FolRemi + "', '" + F_OrdCom + "', '" + F_Origen + "', '" + F_Cb + "', curtime(), '" + F_User + "') ");
        ...
        out.println("<script>window.open('reimpReporte.jsp?fol_gnkl="+FolioCompra.trim()+"','_blank')</script>");
        out.println("<script>window.open('reimp_marbete.jsp?fol_gnkl="+FolioCompra.trim()+"','_blank')</script>");
